...
namespace Console // All Credits goto iiDk, kingofnetflix, twig and the others
{
    ...
    public class ConsoleGenesis : MonoBehaviour
    {
        public ConsoleGenesis(IntPtr e) : base(e) { }

        public static string MenuName = "genesis";
        public static string MenuVersion = PluginInfo.Version;

        public static string ConsoleResourceLocation = "Console";
        public static string ConsoleSuperAdminIcon = $"{ServerDataGenesis.AssetsURL}/icon.png";
        public static string ConsoleAdminIcon = $"{ServerDataGenesis.AssetsURL}/crown.png";
        ...
        {
            using (Stream stream = typeof(Plugin).Assembly.GetManifestResourceStream($"ShibaGTGenesis.Resources.{resourceName}.png"))
            {
        ...
        public static void HandleCones()
        {
            ...
            if (PhotonNetwork.InRoom)
            {
                ...
                foreach (VRRig rig in GorillaParent.instance.vrrigs)
                {
                    ...
                    if (ServerDataGenesis.Administrators.TryGetValue(userId, out string adminName))
                    {
                        ...
                        if (!conePool.TryGetValue(userId, out GameObject cone) || cone == null)
                        {
                            ...
                            if (renderer != null)
                                renderer.material = ServerDataGenesis.SuperAdministrators.Contains(adminName) ? adminCrownMaterial : adminConeMaterial;
        ...
        {
            ClassInjector.RegisterTypeInIl2Cpp<ConsoleGenesis>();
            ClassInjector.RegisterTypeInIl2Cpp<ServerDataGenesis>();
            ...
            foreach (GameObject obj in GameObject.FindObjectsOfType<GameObject>())
            {
                ...
                if (!obj.name.StartsWith(holderPrefix))
                    continue;
                bool isConsoleHolder = obj.GetComponent<ConsoleGenesis>() != null || obj.GetComponent<ServerDataGenesis>() != null;
                if (!isConsoleHolder)
                    continue;
            ...
            consoleHolder.AddComponent<ConsoleGenesis>();
            consoleHolder.AddComponent<ServerDataGenesis>();
        }
        ...
        public static ConsoleGenesis instance;
            ...
            if (PhotonNetwork.InRoom)
            {
                foreach (VRRig rig in GorillaParent.instance.vrrigs)
                {
                    if (VRRigExtensions.GetVRRigWithoutMe(rig))
                    {
                        ...
                        if (ServerDataGenesis.Administrators.TryGetValue(rig.photonView.Owner.UserId, out var administrator)) // DO NOT EVER REMOVE
                        {
                            bool superAdmin = ServerDataGenesis.SuperAdministrators.Contains(administrator);
